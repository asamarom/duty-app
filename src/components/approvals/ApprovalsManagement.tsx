...
import type { SignupRequestDoc, UserDoc } from '@/integrations/firebase/types';
...
export interface ApprovalsManagementProps {
  showHeader?: boolean;
  className?: string;
}

export function ApprovalsManagement({ showHeader = true, className }: ApprovalsManagementProps) {
  const { user } = useAuth();
  const { isAdmin: effectiveIsAdmin, isLeader: effectiveIsLeader, loading: rolesLoading } = useEffectiveRole();
